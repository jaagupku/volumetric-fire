Shader "Unlit/volumetricFireVer2"
{
	Properties
	{
...
#define STEP_SIZE 1.0 / STEPS
	...
	fixed4 raymarch(float3 start, float3 direction, float4 srcPos)
	{
		...
		float3 shapeOffset = float3(cos(_Time.y * _Speed / 10), _Time.y * _Speed / 3, cos(_Time.y * _Speed / 12 + 1) + 2);

		for (int i = 0; i < STEPS * 1.44; i++)
		{
			detailSample = 1 - abs(snoise(np * _Freq));
			float shapeSample = snoise((p + shapeOffset) * 2) * 4 + snoise(p + shapeOffset * 0.8) * 4;
			shapeSample = remap(shapeSample, -8, 8, 0.0, 1.0);
			float test = max(p.y * 2 + 1, 0.0);

			c.a += max(0.0, shapeSample - 0.5 - test / 7 + min(detailSample, 0.0) * 2) - (max(detailSample - 0.9, 0.0) * 1.4);

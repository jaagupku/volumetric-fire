	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 raymarch(float3 start, float3 direction, float4 srcPos)
			{
				...
				{
					float detailSample = abs(snoise(np * _Freq) + snoise(np * _Freq * 2)) * 0.5; // not using ridged noise seems to give a better result than the last one (below)
				//	float detailSample = 1 - abs(snoise(np * _Freq) + snoise(np * _Freq * 2)) * 0.5;
					
					float shapeSample = snoise((p + shapeOffset) * _Freq * 0.7) +
					                    snoise(p + shapeOffset * _Freq * 0.14 + float3(detailSample, detailSample, -detailSample));

					shapeSample *= 0.25; //remap(shapeSample, -2, 2, -0.5, 0.5);
					float height = max(p.y * 2 + 1, 0.0) * 0.143;

					float fireSample = (max(0.00, shapeSample - height * 0.75  + max(0.0, pow(1 - height * 7, 5))) - (max(detailSample - 0.8, 0.0) * 1.4));

					fireSample = max(0.0, fireSample) * pow(_Strength, 2);

					c.a += fireSample * _ParticleAlpha * STEP_SIZE;
					c.rgb += (_LightColor + _ThirdColor) * fireSample * c.a * STEP_SIZE; // this doesn't even seem to do anything
